...
\subsection{Perché si usa solo l'energia di invocazione}\label{sec:coldstart}

Il costo di cold-start ($C_{\text{cs}}$) è il tempo/energia necessario
per avviare un nuovo container quando non è disponibile un'istanza
calda.  Esso dipende dall'\emph{ambiente di esecuzione} (runtime
Python, runtime C/Alpine) ed è \emph{transitorio}: per le invocazioni
successive (\emph{warm start}) non vengono sostenuti.

Se si includesse $C_{\text{cs}}$ nella comparazione Pareto, il runtime
C dominerebbe sistematicamente quello Python ($C_{\text{cs,C}} \approx
0.008\,\text{J}$ contro $C_{\text{cs,Py}} \approx 0.069\,\text{J}$),
anche quando le varianti Python approssimate sono più economiche in
invocazione.  L'obiettivo del fronte di Pareto è confrontare le
proprietà \emph{algoritmiche} delle varianti, non il costo della
piattaforma.

\subsection{Implementazione Go}

\begin{lstlisting}[style=go, caption={Filtro Pareto --- \texttt{variant\_selection.go}}]
func paretoFilter(candidates []evaluatedVariant) []evaluatedVariant {
    ...
    for i := 0; i < n; i++ {
        for j := 0; j < n; j++ {
            ...
            // j domina i se >= su entrambi e < su almeno uno
    ...
    for i, v := range candidates {
        if !dominated[i] { pareto = append(pareto, v) }
    }
    return pareto
}
\end{lstlisting}
