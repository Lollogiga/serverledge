...
# =====================================================
sep
# La selezione varianti è automatica, guidata dalla carbon intensity.
# Fronte di Pareto = {c} (punto singolo, err=0 per tutte le varianti).
# Qualunque CI porta alla stessa scelta: c (energia minima, err=0).

# =====================================================
# INVOKE 1 — default zone (da serverledge-conf.yaml)
# =====================================================
sep
log_info "INVOKE 1 — default zone  (atteso: c)"
$SERVERLEDGE invoke \
  --function fibonacci \
  --param n:10
...
# λ alto → priorità qualità → c (unico pareto-ottimale)
# =====================================================
sep
log_info "INVOKE 2 — ci-zone=NO-NO5  (rete pulita — atteso: c)"
$SERVERLEDGE invoke \
  --function fibonacci \
  --param n:10 \
  --ci-zone NO-NO5
...
# λ basso → priorità energia → c (unico pareto-ottimale)
# =====================================================
sep
log_info "INVOKE 3 — ci-zone=PL  (rete sporca — atteso: c)"
$SERVERLEDGE invoke \
  --function fibonacci \
  --param n:10 \
  --ci-zone PL
